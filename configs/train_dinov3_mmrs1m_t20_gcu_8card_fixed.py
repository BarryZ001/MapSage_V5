# DINOv3 + MMRS-1M 8卡分布式训练配置文件 - 燧原T20 GCU版本（修复版）
...
data_preprocessor = dict(
    ...
    std=[58.395, 57.12, 57.375],
    to_rgb=True,  # 统一使用to_rgb而不是bgr_to_rgb
    pad_val=0,
    seg_pad_val=255,
    size=crop_size
)
...
model = dict(
    ...
    backbone=dict(
        ...
        out_indices=(23,),  # 输出最后一层 - 确认DINOv3-L实际层数
        final_norm=True,
        drop_path_rate=0.1,
...
train_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        task_type='segmentation',  # 修复：classification -> segmentation
        modality='optical',
...
# 混合精度训练 - 使用更保守的loss_scale
fp16 = dict(loss_scale='dynamic')  # 修复：使用动态loss_scale而不是固定值
...
print(f"🚀 DINOv3 + MMRS-1M 燧原T20 GCU 8卡分布式训练配置已加载（修复版）")
